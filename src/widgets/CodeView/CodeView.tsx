  ...
  const handleMount: OnMount = useCallback(
    (editor, monaco) => {
      ...
      editor.onDidChangeModelContent(() => {
        ...
        const newLines: CodeSegment[][] = value
          .split("\n")
          .map((line) => (line.length > 0 ? [["p", line]] : []));
        onFileChange?.(newLines);
      });

      if (cursors.length > 0) {
        const decorations: monacoEditor.IModelDeltaDecoration[] = cursors.map(
          (c) => {
      ...
      if (comments.length > 0) {
        const commentDecorations: monacoEditor.IModelDeltaDecoration[] =
          comments.map((c) => ({
    ...
    <div className={cls.root}>
      <div className={cls.head}>
        <div className={cls.crumb}>
          {file.path.map((p, i) => (
            <span key={i}>
              {p} <span className={cls.crumbSep}>/</span>
            </span>
          ))}
      ...
      <div className={cls.foot}>
        <span className={cls.live}>
          <span className={cls.liveDot} /> {cursors.length + 1} editing
        </span>
